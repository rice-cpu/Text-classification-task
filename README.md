...
统一编号对齐：为了防止训练集、验证集和测试集因为个别分类缺失导致编号错位，代码里写了一个固定的 15 分类标准顺序列表。所有数据集在换算数字标签时，对应的都是一模一样的固定编号，保证测试时一致。
## 四、代码运行流程
1. 第一步：开始训练模型
...
测试集有效样本总数：1064 条  
模型预测正确样本数：909 条  
测试集最终准确率 (Accuracy)：85.43%

结果简析  
模型在训练期间的dev_3k.txt验证集上拿到的最高准确率是86.30%而在test_1k.txt测试集上达到85.43%。
这两个数字高度接近，证明了以下两点：
1. 权重质量高  
2. 泛化能力好
