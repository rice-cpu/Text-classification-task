...
2. 第二步：在独立测试集上测试
训练全部结束后，在终端运行test.py 脚本进行测试  
这个脚本会完全独立启动，自动去加载刚刚训练时保存好的best_checkpoint.pt 权重。然后把从来没有参与过训练、也没有参与过验证的1064条独立测试集数据（data/test_1k.txt）送进模型进行预测，最后和正确答案做对比，计算出最终的全局准确率。
## 五、实验最终结果
运行测试脚本test.py 后，控制台最终输出的真实数据如下：  
测试集有效样本总数：1064条  
模型预测正确样本数：900条  
